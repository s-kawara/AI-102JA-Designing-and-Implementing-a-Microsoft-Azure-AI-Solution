...
13. **スケジュール**は **1 度**に設定したままにします。
14. **「詳細オプション」** を展開し、**「Base-64 エンコード キー」** オプションが選択されていることを確認します (通常、キーをエンコードするとインデックスがより効率的になります)。
...
1. Azure Cognitive Searchリソースの **「概要」** ページの上部で、**「検索エクスプローラー」** を選択します。
